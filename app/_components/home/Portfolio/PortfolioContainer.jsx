...
export default function PortfolioContainer({ project }) {
  ...
  return (
    <>
      <div className="group relative overflow-hidden rounded-[2.5rem] bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-white/5 shadow-2xl hover:shadow-[0_20px_50px_rgba(0,0,0,0.12)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.3)] transition-all duration-500">
        <div className="flex flex-col md:flex-row h-full">
          {/* Visual Side */}
          <div
            className={`relative flex-1 order-1 md:order-2 min-h-[300px] md:min-h-0 bg-gradient-to-br ${project.gradient} p-8 md:p-12 flex items-center justify-center overflow-hidden`}
          >
          ...
          <div className="flex-1 order-2 md:order-1 p-8 md:p-14 flex flex-col justify-center relative z-10 bg-white dark:bg-[#0a0a0a]">
            {/* Decorative Line */}
            <div
              className={`w-12 h-1.5 rounded-full mb-8 ${project.color.replace(
                "bg-",
                "bg-"
              )}`}
            ></div>
            ...
            <div className="mb-4">
              <span
                className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold tracking-widest uppercase ${project.color
                  .replace("bg-", "text-")
                  .replace("600", "500")} bg-opacity-10 bg-current`}
              >
                <span
            ...
            <div className="flex items-center gap-6">
              <Button
                type="primary"
                className="!px-8 !py-4 !text-lg shadow-xl shadow-primary-default/20 hover:shadow-primary-default/40"
